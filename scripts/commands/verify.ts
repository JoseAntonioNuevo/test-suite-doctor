 ...
 *
 * Standalone usage:
 *   test-suite-doctor verify --baseline .test-doctor/report.json
 *   test-suite-doctor verify --mutation --mutate "src/billing/**" --mutation-floor 80
 */
...
export async function verifyCommand(args = process.argv.slice(2)): Promise<void> {
  ...
  try {
    ...
    fail(`--cwd does not exist: ${resolve(values.cwd!)}`);
  }
