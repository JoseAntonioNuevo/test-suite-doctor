 ...
 *
 * Standalone usage:
 *   test-suite-doctor minimize --report .test-doctor/report.json \
...
export function minimizeCommand(args = process.argv.slice(2)): void {
  ...
  try {
    ...
    frontierFloors = values.frontier == null
      ? []
      : [...new Set(values.frontier.split(",").map((value, index) =>
          parseFraction(`--frontier entry ${index + 1}`, value),
        ))].sort((a, b) => b - a);
  } catch (error) {
